...
public class SegmentsItem extends TrackItBaseType implements FolderTreeItem {
	private static ImageIcon icon = ImageUtilities.createImageIcon("segments_16.png");;
	private DocumentItem parent;
	
	public SegmentsItem(DocumentItem parent) {
		this.parent = parent;
	}
	...
	public String getFolderTreeItemName() {
		return getMessage("folderView.label.segments");
	}
	...
	public void publishSelectionEvent(EventPublisher publisher) {
		EventManager.getInstance().publish(publisher, Event.SEGMENTS_SELECTED, parent, parent.getSegments());
	}
	...
	public List<ActionType> getSupportedActions() {
		...
		supportedActions.add(ActionType.CLEAR_SEGMENTS);
