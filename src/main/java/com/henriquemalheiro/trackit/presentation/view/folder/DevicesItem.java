...
public class DevicesItem extends TrackItBaseType implements FolderTreeItem {
	private static ImageIcon icon = ImageUtilities.createImageIcon("devices_16.png");
	private DocumentItem parent;
	
	public DevicesItem(DocumentItem parent) {
		this.parent = parent;
	}
	...
	public String getFolderTreeItemName() {
		return getMessage("folderView.label.devices");
	}
	...
	public void publishSelectionEvent(EventPublisher publisher) {
		EventManager.getInstance().publish(publisher, Event.DEVICES_SELECTED, parent, parent.getDevices());
	}
