...
public class LapsItem extends TrackItBaseType implements FolderTreeItem {
	private static ImageIcon icon = ImageUtilities.createImageIcon("laps_16.png");;
	
	private DocumentItem parent;
	
	public LapsItem(DocumentItem parent) {
		this.parent = parent;
	}
	...
	public String getFolderTreeItemName() {
		return getMessage("folderView.label.laps");
	}
	...
	public void publishSelectionEvent(EventPublisher publisher) {
		EventManager.getInstance().publish(publisher, Event.LAPS_SELECTED, parent, parent.getLaps());
	}
