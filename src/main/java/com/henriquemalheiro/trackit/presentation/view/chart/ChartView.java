...
import com.henriquemalheiro.trackit.business.operation.AltitudeSmoothingOperation;
...
public class ChartView extends JPanel implements EventListener, EventPublisher {
	private static final long serialVersionUID = -1172194726510817403L;
	...
	private void updateDistancesAndDurations() {
		...
		if (trackpoints.size() > 0) {
			double initialTimestamp = trackpoints.get(0).getTimestamp()
					.getTime();

			for (int i = 0; i < trackpoints.size(); i++) {
				distances[i] = trackpoints.get(i).getDistance();

				durations[i] = trackpoints.get(i).getTimestamp().getTime()
						- initialTimestamp;
				durations[i] /= 1000.0;
			}
		}
	...
	TrackSegment selectByDistance(double initialDistance, double finalDistance) {
		...
		if (!finalTrackpoint.equals(initialTrackpoint)) {
			...
			segment.setTrackpoints(item.getTrackpoints().subList(
					item.getTrackpoints().indexOf(initialTrackpoint),
	...
	private void highlightTrackpoint(Trackpoint trackpoint) {
		if (trackpoint != null) {
			...
			EventManager
					...
					.publish(
							null,
							com.henriquemalheiro.trackit.presentation.event.Event.TRACKPOINT_HIGHLIGHTED,
							null);
		}
	...
	private void init() {
		...
		showSpeedProfile = TrackIt.getPreferences().getBooleanPreference(
				Constants.PrefsCategories.CHART, null,
				Constants.ChartPreferences.SHOW_SPEED_PROFILE, true);
		showElevationProfile = TrackIt.getPreferences().getBooleanPreference(
				Constants.PrefsCategories.CHART, null,
				Constants.ChartPreferences.SHOW_ELEVATION_PROFILE, false);
		showHeartRateProfile = TrackIt.getPreferences().getBooleanPreference(
				Constants.PrefsCategories.CHART, null,
				Constants.ChartPreferences.SHOW_HEART_RATE_PROFILE, false);
		showCadenceProfile = TrackIt.getPreferences().getBooleanPreference(
				Constants.PrefsCategories.CHART, null,
				Constants.ChartPreferences.SHOW_CADENCE_PROFILE, false);
		showPowerProfile = TrackIt.getPreferences().getBooleanPreference(
				Constants.PrefsCategories.CHART, null,
				Constants.ChartPreferences.SHOW_POWER_PROFILE, false);
		showTemperatureProfile = TrackIt.getPreferences().getBooleanPreference(
				Constants.PrefsCategories.CHART, null,
				Constants.ChartPreferences.SHOW_TEMPERATURE_PROFILE, false);

		showSmoothData = TrackIt.getPreferences().getBooleanPreference(
				Constants.PrefsCategories.CHART, null,
				Constants.ChartPreferences.SHOW_SMOOTHED_DATA, false);

		showGrade = TrackIt.getPreferences().getBooleanPreference(
				Constants.PrefsCategories.CHART, null,
	...
	private void display(DocumentItem item) {
		display(item.getTrackpoints(), new ArrayList<Trackpoint>(),
				item.getSegments());
		showCoursePoints(item);
	}

	private void display(DocumentItem item, DocumentItem selection) {
		List<Trackpoint> selectionTrackpoints = (selection == null ? Collections
				.<Trackpoint> emptyList() : selection.getTrackpoints());
	...
	private void showCoursePoints(DocumentItem item) {
		...
		if (rootParent != null) {
			chart.setCoursePoints(getFilteredCoursePoints(rootParent
					.getCoursePoints()));
		}
	}

	private List<CoursePoint> getFilteredCoursePoints(
	...
	private void display(List<Trackpoint> trackpoints,
			List<Trackpoint> selection) {
		display(trackpoints, selection, new ArrayList<TrackSegment>());
	}

	private void display(List<Trackpoint> trackpoints,
			List<Trackpoint> selection, List<TrackSegment> segments) {
		...
		if ((firstTrackpoint.getDistance() == null && firstTrackpoint
				.getTimestamp() == null)
		...
		for (int i = 0; i < trackpoints.size(); i++) {
			...
			if (trackpoint.getHeartRate() != null) {
				minHeartRate = Math
						.min(minHeartRate, trackpoint.getHeartRate());
				maxHeartRate = Math
						.max(maxHeartRate, trackpoint.getHeartRate());
			...
			double xValue = (chartType == ChartType.DISTANCE ? trackpoint
					.getDistance()
		...
		if (showElevationProfile) {
			if (showSmoothData) {
				double elevationSmoothingFactor = TrackIt
						.getPreferences()
						.getDoublePreference(
								...
								34.0);
				AltitudeSmoothingOperation.smoothing(elevationData,
						elevationSmoothingFactor, true);
			}
			scale = new Scale(minElevation, maxElevation, 5, true);
			series = new DataSeries("Elevation", scale, Unit.METER,
					ColorScheme.LIGHT_GREEN, true, elevationData);

			if (!selection.isEmpty()) {
				series.setSelection(
						trackpoints.indexOf(selection.get(0)),
						trackpoints.indexOf(selection.get(selection.size() - 1)));
			}

			if (highlight != null) {
				int highlightIndex = trackpoints.indexOf(highlight);
				series.setHighlight(Pair.create(
						elevationData[highlightIndex][0],
						elevationData[highlightIndex][1]));
			} else {
				series.setHighlight(null);
			}

			if (!segments.isEmpty()) {
				List<TrackSegment> filteredSegments = Utilities.filter(
						segments, new Predicate<TrackSegment>() {
							@Override
				...
				for (TrackSegment segment : filteredSegments) {
					int startIndex = trackpoints.indexOf(segment
							.getTrackpoints().get(0));
					int endIndex = trackpoints.indexOf(segment.getTrackpoints()
							.get(segment.getTrackpoints().size() - 1));
					Pair<Integer, Integer> shading = Pair.create(startIndex,
							endIndex);
		...
		if (showHeartRateProfile) {
			...
			if (showSmoothData) {
				double heartRateSmoothingFactor = TrackIt
						.getPreferences()
						.getDoublePreference(
								Constants.PrefsCategories.CHART,
								null,
				...
				if (heartRateSmoothingFactor > 0.0) {
					AltitudeSmoothingOperation.smoothing(heartRateData,
							heartRateSmoothingFactor, true);
				}
			}

			series = new DataSeries("Heart Rate", scale, Unit.BPM,
					ColorScheme.LIGHT_RED, false, heartRateData);
			dataSeries.add(series);
		}
		...
		if (showSpeedProfile) {
			...
			if (showSmoothData) {
				double speedSmoothingFactor = TrackIt
						.getPreferences()
						.getDoublePreference(
								Constants.PrefsCategories.CHART,
								null,
				...
				if (speedSmoothingFactor > 0.0) {
					AltitudeSmoothingOperation.smoothing(speedData,
							speedSmoothingFactor, true);
				}
			}

			series = new DataSeries("Speed", scale, Unit.KILOMETER_PER_HOUR,
					ColorScheme.LIGHT_BLUE, false, speedData);
			if (!selection.isEmpty()) {
				series.setSelection(
						trackpoints.indexOf(selection.get(0)),
						trackpoints.indexOf(selection.get(selection.size() - 1)));
			...
			if (highlight != null) {
				int highlightIndex = trackpoints.indexOf(highlight);
				series.setHighlight(Pair.create(
						speedData[highlightIndex][0],
						speedData[highlightIndex][1]));
			} else {
				series.setHighlight(null);
			}
		...
		if (showPowerProfile) {
			...
			if (showSmoothData) {
				double powerSmoothingFactor = TrackIt
						.getPreferences()
						.getDoublePreference(
								Constants.PrefsCategories.CHART,
								null,
				...
				if (powerSmoothingFactor > 0.0) {
					AltitudeSmoothingOperation.smoothing(powerData,
							powerSmoothingFactor, true);
				}
			}

			series = new DataSeries("Power", scale, Unit.WATT,
					ColorScheme.DARK_YELLOW, false, powerData);
			dataSeries.add(series);
		}
		...
		if (showCadenceProfile) {
			...
			if (showSmoothData) {
				double cadenceSmoothingFactor = TrackIt
						.getPreferences()
						.getDoublePreference(
								Constants.PrefsCategories.CHART,
								null,
				...
				if (cadenceSmoothingFactor > 0.0) {
					AltitudeSmoothingOperation.smoothing(cadenceData,
							cadenceSmoothingFactor, true);
				}
			}

			series = new DataSeries("Cadence", scale, Unit.RPM,
					ColorScheme.LIGHT_ORANGE, false, cadenceData);
			dataSeries.add(series);
		}
		...
		if (showTemperatureProfile) {
			...
			if (showSmoothData) {
				AltitudeSmoothingOperation.smoothing(temperatureData, 74.0,
						true);
			}
			series = new DataSeries("Temperature", scale, Unit.DEGREES_CELSIUS,
					ColorScheme.DARK_YELLOW, false, temperatureData);
			dataSeries.add(series);
		}
	...
	private JToolBar createJToolBar() {
		...
		String mode = TrackIt.getPreferences().getPreference(
				Constants.PrefsCategories.CHART, null,
				Constants.ChartPreferences.CHART_MODE,
				ChartMode.SELECTION.name());
		ChartMode lastChartMode = ChartMode.valueOf(mode);

		toggleButton = new JToggleButton();
		toggleButton.setActionCommand("SelectionMode");
		toggleButton
				.setToolTipText(getMessage("chartView.tooltip.selectionMode"));
		...
		toggleButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				TrackIt.getPreferences()
						.setPreference(Constants.PrefsCategories.CHART, null,
								Constants.ChartPreferences.CHART_MODE,
								chartMode.name());
			}
		});
		...
		distanceToggleButton
				.setToolTipText(getMessage("chartView.tooltip.showDistance"));
		...
		distanceToggleButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				TrackIt.getPreferences()
						.setPreference(Constants.PrefsCategories.CHART, null,
								Constants.ChartPreferences.CHART_TYPE,
								chartType.name());
			}
		});
		...
		timeToggleButton
				.setToolTipText(getMessage("chartView.tooltip.showTime"));
		...
		toggleButton
				.setToolTipText(getMessage("chartView.tooltip.showElevationProfile"));
		toggleButton.setFocusable(false);
		toggleButton.setSelected(showElevationProfile);
		// toggleButton.setEnabled(false);//58406
		if (item != null && item.getTrackpoints().get(0).getAltitude() == null){
		...
		toggleButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				TrackIt.getPreferences().setPreference(
						Constants.PrefsCategories.CHART, null,
						Constants.ChartPreferences.SHOW_ELEVATION_PROFILE,
						showElevationProfile);
			}
		});
		imageURL = ChartView.class
				.getResource("/icons/elevation_profile_16.png");
		...
		toggleButton
				.setToolTipText(getMessage("chartView.tooltip.showSpeedProfile"));
		toggleButton.setFocusable(false);
		toggleButton.setSelected(showSpeedProfile);
		if (item != null && item.getTrackpoints().get(0).getSpeed() == null){
		...
		toggleButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				TrackIt.getPreferences().setPreference(
						Constants.PrefsCategories.CHART, null,
						Constants.ChartPreferences.SHOW_SPEED_PROFILE,
						showSpeedProfile);
			}
		});
		...
		toggleButton
				.setToolTipText(getMessage("chartView.tooltip.showHeartRateProfile"));
		toggleButton.setFocusable(false);
		toggleButton.setSelected(showHeartRateProfile);
		if (item != null && item.getTrackpoints().get(0).getHeartRate() == null){
		...
		toggleButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				TrackIt.getPreferences().setPreference(
						Constants.PrefsCategories.CHART, null,
						Constants.ChartPreferences.SHOW_HEART_RATE_PROFILE,
						showHeartRateProfile);
			}
		});
		...
		toggleButton
				.setToolTipText(getMessage("chartView.tooltip.showCadenceProfile"));
		toggleButton.setFocusable(false);
		toggleButton.setSelected(showCadenceProfile);
		if (item != null && item.getTrackpoints().get(0).getCadence() == null){
		...
		toggleButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				TrackIt.getPreferences().setPreference(
						Constants.PrefsCategories.CHART, null,
						Constants.ChartPreferences.SHOW_CADENCE_PROFILE,
						showCadenceProfile);
			}
		});
		...
		toggleButton
				.setToolTipText(getMessage("chartView.tooltip.showPowerProfile"));
		toggleButton.setFocusable(false);
		toggleButton.setSelected(showPowerProfile);
		if (item != null && item.getTrackpoints().get(0).getPower() == null){
		...
		toggleButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				TrackIt.getPreferences().setPreference(
						Constants.PrefsCategories.CHART, null,
						Constants.ChartPreferences.SHOW_POWER_PROFILE,
						showPowerProfile);
			}
		});
		...
		toggleButton
				.setToolTipText(getMessage("chartView.tooltip.showTemperatureProfile"));
		toggleButton.setFocusable(false);
		toggleButton.setSelected(showTemperatureProfile);
		if (item != null && item.getTrackpoints().get(0).getTemperature() == null){
		...
		toggleButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				TrackIt.getPreferences().setPreference(
						Constants.PrefsCategories.CHART, null,
						Constants.ChartPreferences.SHOW_TEMPERATURE_PROFILE,
						showTemperatureProfile);
			}
		});
		...
		toggleButton
				.setToolTipText(getMessage("chartView.tooltip.showCoursePoints"));
		toggleButton.setFocusable(false);
		...
		toggleButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				TrackIt.getPreferences().setPreference(
						Constants.PrefsCategories.CHART, null,
						Constants.ChartPreferences.SHOW_COURSE_POINTS,
						showCoursePoints);
			}
		});
		imageURL = ChartView.class
				.getResource("/icons/coursepoints/Generic.png");
		...
		imageURL = ChartView.class
				.getResource("/icons/coursepoints/Summit.png");
		...
		toggleButton
				.setToolTipText(getMessage("chartView.tooltip.showDescents"));
		toggleButton.setFocusable(false);
		...
		imageURL = ChartView.class
				.getResource("/icons/coursepoints/Valley.png");
		...
		toggleButton
				.setToolTipText(getMessage("chartView.tooltip.showSmoothedData"));
		toggleButton.setFocusable(false);
		...
		toggleButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				TrackIt.getPreferences().setPreference(
						Constants.PrefsCategories.CHART, null,
						Constants.ChartPreferences.SHOW_SMOOTHED_DATA,
						showSmoothData);
			}
		});
	...
	@Override
	public void process(
			com.henriquemalheiro.trackit.presentation.event.Event event,
			DocumentItem item) {
		clearHighlight();
		switch (event) {
		case DOCUMENT_ADDED:
	...
	void resetZoom() {
		...
		if (rootItem != null) {
			...
			EventManager
					...
					.publish(
							null,
							com.henriquemalheiro.trackit.presentation.event.Event.ZOOM_TO_ITEM,
							rootItem);
		}
	...
	private void displayHighlightedTrackpoint(Trackpoint trackpoint) {
		if (trackpoint != null && item != null
				&& item.getTrackpoints().contains(trackpoint)) {
	...
	public DocumentItem getRootItem(DocumentItem item) {
		...
		while (rootItem.getParent() != null && !rootItem.isActivity()
				&& !rootItem.isCourse()) {
