...
public class ChartView extends JPanel implements EventListener, EventPublisher {
	...
	private void highlightTrackpoint(Trackpoint trackpoint) {
		if (trackpoint != null) {
			...
			EventManager.getInstance().publish(null,
					com.henriquemalheiro.trackit.presentation.event.Event.TRACKPOINT_HIGHLIGHTED, null);
		}
	...
	@Override
	public void process(com.henriquemalheiro.trackit.presentation.event.Event event, DocumentItem parent,
	...
	void resetZoom() {
		...
		if (rootItem != null) {
			...
			EventManager.getInstance().publish(null, com.henriquemalheiro.trackit.presentation.event.Event.ZOOM_TO_ITEM,
					rootItem);
		}
	...
	void zoomItem(Trackpoint trackpoint) {
		EventManager.getInstance().publish(this, com.henriquemalheiro.trackit.presentation.event.Event.ZOOM_TO_ITEM,
				trackpoint);
	}
}
