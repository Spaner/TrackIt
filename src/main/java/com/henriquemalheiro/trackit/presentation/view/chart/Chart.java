...
//		}
	...
	private void drawGrid(Graphics2D graphics) {
		...
		for (int i = 0; i < defaultSteps; i++) {
			graphics.drawLine(0, (int) (getHeight() - (pixelsPerStep * i)),
					getWidth(), (int) (getHeight() - (pixelsPerStep * i)));
		}
	}
	...
	private void drawData(Graphics2D graphics, DataSeries series) {
		if (dataAvailable(series)) {
			...
			drawSegment(graphics, series, data, MapPainterStyle.REGULAR);
	...
	private Point2D.Double getProfilePoint(double xi, double yi, Scale yScale) {
		...
		double x = (xi - chart.scale.getMinValue())
				* getWidth() / (chart.scale.getMaxValue() - chart.scale.getMinValue() + 1);
		double y = getHeight() - (yi - yScale.getMinValue()) * pixelsPerMeter;
		
		return new Point2D.Double(x, y);
	}
	
	private void drawProfileShading(Graphics2D graphics, Path2D chartLine,
			DataSeries series, double[][] data, MapPainterStyle style) {
		
		double xi = (data[0][0] - chart.scale.getMinValue())
				* getWidth() / (chart.scale.getMaxValue() - chart.scale.getMinValue() + 1);
		double x = xi + (data[data.length - 1][0] - data[0][0])
				* getWidth() / (chart.scale.getMaxValue() - chart.scale.getMinValue() + 1);
		double y = getHeight();
	...
	private void drawTrackpoint(Graphics2D graphics, DataSeries series, double xValue, double yValue,
		...
		int x = (int) ((xValue - chart.scale.getMinValue())
				* getWidth() / (chart.scale.getMaxValue() - chart.scale.getMinValue()));
	...
	private void drawCoursePoint(Graphics2D graphics, CoursePoint coursePoint) {
		...
		int x = (int) ((coursePoint.getTrackpoint().getDistance() - chart.scale.getMinValue())
				* getWidth() / (chart.scale.getMaxValue() - chart.scale.getMinValue() + 1));
	...
	private double getDataSeriesVerticalSpace() {
		double dataSeriesVerticalSpace = TrackItPreferences.getInstance()
	...
	private double getDefaultSteps() {
		double defaultSteps = TrackItPreferences.getInstance()
				.getDoublePreference(Constants.PrefsCategories.CHART, null,
