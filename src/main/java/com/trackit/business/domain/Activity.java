...
//	private Boolean unsavedChanges;         			// 12335: 2015-07-24
	...
	private DifficultyLevelType difficulty;				// 12335: 2017-03-16
	private TrackConditionType trackCondition;			// 12335: 2017-03-16
	private CircuitType circuitType;					// 12335: 2017-03-16
...
//		unsavedChanges = false;    			 		// 12335 : 2015-07-24
		...
		difficulty     = DifficultyLevelType.UNDEFINED;
		trackCondition = TrackConditionType.UNDEFINED;
...
//	}
	...
	private void setTrackCondition( TrackConditionType trackCondition, boolean isChange) {
		if ( this.trackCondition != trackCondition) {
			...
			for( Session session: sessions)
				session.setTrackCondition( trackCondition);
			for( Lap lap: laps)
	...
	//12335: 2018-07-02
	public CircuitType setCircuitType() {
		this.circuitType = CircuitType.OPEN;
		...
		for( Session session: sessions )
			session.setCircuitType( tolerance);
		for( Lap lap: laps)
...
//		System.out.println( "activity start and end times " + this.startTime + "  " + this.endTime);
//		System.out.println( "call Start, end: " + startTime + "  " + endTime + "  this is " + this.getId());
//		System.out.println( "No of tracks: " + tracks.size());
//		for( Track tr: tracks)
//			System.out.println( "Activity track" + tr.getStartTime() + "  " + tr.getEndTime());
	...
	public void publishSelectionEvent(EventPublisher publisher) {
		EventManager
				...
				.publish(
						publisher,
						com.trackit.presentation.event.Event.ACTIVITY_SELECTED,
						this);
	}
...
//		setUnsavedTrue();				// 12335 : 2016-10-03
	...
	public void publishUpdateEvent(EventPublisher publisher) {
		EventManager
				...
				.publish(
						publisher,
						com.trackit.presentation.event.Event.ACTIVITY_UPDATED,
						this);
	}
	...
	public Activity copy() {
		...
		activity.oldName = this.oldName;
		// trackpoints
		for( Trackpoint point: this.getTrackpoints())
			activity.getTrackpoints().add( point.clone( activity));
		// laps
		for( Lap lap: this.getLaps())
			activity.getLaps().add( ((ActivityLap) lap).clone( activity));
		// tracks
		for( Track track: this.getTracks() )
			activity.getTracks().add( ((ActivityTrack) track).clone( this));
		// sessions
		for( Session session: this.getSessions()) {
			activity.getSessions().add( session.clone( this));
		}

		// other settings
		activity.setSportAndSubSport( this.getSport(), this.getSubSport());
		activity.setDifficulty( this.getDifficulty());
		activity.setTrackCondition( this.getTrackCondition());
	...
	public Activity clone() {
		return clone( this.getParent(), "");
	}
	
	public Activity clone( GPSDocument parent, String nameSuffix) {
		Activity activity = new Activity();
		activity.name    = this.name    + nameSuffix;
		activity.oldName = this.oldName + nameSuffix;
		activity.setParent( parent);
		activity.setFilepath( parent.getFileName());		
		// trackpoints
		for( Trackpoint point: this.getTrackpoints())
			activity.getTrackpoints().add( point.clone( activity));
		// laps
		...
		for( Pause pause: this.getPauses() )
			activity.getPauses().add( pause.clone());
		// other settings
		activity.setSportAndSubSport( this.getSport(), this.getSubSport());
		activity.setDifficulty( this.getDifficulty());
		activity.setTrackCondition( this.getTrackCondition());
