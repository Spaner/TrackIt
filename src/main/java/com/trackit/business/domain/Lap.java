...
import com.trackit.presentation.view.map.provider.here.routes.routingcalculatematrix.SourceSupplierNoteTypeType;
...
public abstract class Lap extends TrackItBaseType implements DocumentItem, FolderTreeItem {
	...
	public Lap() {
		...
		this.circuitType = CircuitType.OPEN; 				//12335: 2017-03-16
		this.difficulty = DifficultyLevelType.UNDEFINED;	//12335: 2017-03-14
		this.trackCondition = TrackConditionType.UNDEFINED;	//12335: 2017-03-14
	...
	//12335: 2017-03-16
	//12335: 2018-07-02
	public void setCircuitType( double tolerance) {
	...
	public void consolidate(ConsolidationLevel level) {
		...
		Double ascentTime                  = 0.0;		//12335: 2017-07-14
		Double descentTime				   = 0.0;		//12335: 2017-07-14
		Float maximumPositiveVerticalSpeed = 0.0f;		//12335: 2017-07-14
		Float maximumNegativeVerticalSpeed = 0.0f;		//12335: 2017-07-14
		...
		for (Trackpoint trackpoint : trackpoints) {
			...
			if (trackpoint.getAltitude() != null && lastAltitude != null) {
				...
				float verticalSpeed = (float) (altitudeDiff / trackpoint.getTimeFromPrevious());	//12335: 2017-07-14

				if (altitudeDiff > 0.0) {
					totalAscent                 += altitudeDiff;
					ascentTime                  += trackpoint.getTimeFromPrevious();							//12335: 2017-07-14
					maximumPositiveVerticalSpeed = Math.max( verticalSpeed, maximumPositiveVerticalSpeed);		//12335: 2017-07-14
				} else if (altitudeDiff < 0.0) {
					totalDescent -= altitudeDiff;
					descentTime  += trackpoint.getTimeFromPrevious();		//12335: 2017-07-14
					maximumNegativeVerticalSpeed = Math.max( -verticalSpeed, maximumNegativeVerticalSpeed);		//12335: 2017-07-14
				}
		...
		if ( getAscentTime() == null || recalculation )
			setAscentTime( ascentTime);
		
		if ( getDescentTime() == null || recalculation )
			setDescentTime( descentTime);
		
		if ( getMaximumPositiveVerticalSpeed() == null || recalculation )
			setMaximumPositiveVerticalSpeed( maximumPositiveVerticalSpeed);
		
		if ( getMaximumNegativeVerticalSpeed() == null || recalculation )
			setMaximumNegativeVerticalSpeed( maximumNegativeVerticalSpeed);
		
		if ( getAveragePositiveVerticalSpeed() == null || recalculation )
			setAveragePositiveVerticalSpeed( (float) (totalAscent/ascentTime)); 
		
		if ( getAverageNegativeVerticalSpeed() == null || recalculation )
			setAverageNegativeVerticalSpeed( (float) (totalDescent/descentTime));
...
//		limit = DocumentManager.getInstance().getDatabase().getPauseThresholdSpeed(sport, subSport, false);
		double limit = 3.6;
		if ( this.getParent().isActivity() )
			limit = ((Activity) this.getParent()).getSubSport().getPauseThresholdSpeed();
		else
			if ( this.getParent().isCourse() )
				limit = ((Course) this.getParent()).getSubSport().getPauseThresholdSpeed();
		return limit;
	}
	
	protected Lap copy( Lap source) {
		// data that is not recalculated
		setTrigger( source.getTrigger());
		setEvent( source.getEvent());
		setEventType( source.getEventType());
		setEventGroup( source.getEventGroup());
		
		setCycles( source.getCycles());
		setStrides( source.getStrides());
		setCalories( source.getCalories());
		setFatCalories( source.getFatCalories());

		setAverageRunningCadence( source.getAverageRunningCadence());
		setMaximumRunningCadence( source.getMaximumRunningCadence());
		setAveragePower( source.getAveragePower());
		setMaximumPower( source.getMaximumPower());
		setIntensity( source.getIntensity());
		setNormalizedPower( source.getNormalizedPower());
		setLeftRightBalance( source.getLeftRightBalance());
		setWork( source.getWork());
		setAverageStrokeDistance( source.getAverageStrokeDistance());
		setSwimStroke( source.getSwimStroke());
		return this;
	}
