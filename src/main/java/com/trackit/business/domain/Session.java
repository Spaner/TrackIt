...
public class Session extends TrackItBaseType implements DocumentItem, FolderTreeItem {
	...
	private Double ascentTime;  					//12335: 2017-07-15
	private Double descentTime;						//12335: 2017-07-15
	...
	public void consolidate(ConsolidationLevel level) {
		...
		double ascentTime = 0.0;		//12335: 2017-07-15
		double descentTime = 0.0;		//12335: 2017-07-15
		double distance = 0.0;
		...
		int totalAscent = 0;
		int totalDescent = 0;
		float maximumPositiveVerticalSpeed = -1.0f;			//12335: 2017-07-15
		float maximumNegativeVerticalSpeed = -1.0f;			//12335: 2017-07-15
		...
		elapsedTime = (endTime.getTime() - startTime.getTime()) * 0.001;	//12335: 2018-05-07
		
		for (Lap lap : getLaps()) {
//			elapsedTime += lap.getElapsedTime();		//12335: 2018-05-07
			timerTime += lap.getTimerTime();
//			pausedTime += lap.getPausedTime();			//12335: 2018-05-07
			movingTime += lap.getMovingTime();
			ascentTime += lap.getAscentTime();			//12335: 2017-07-15
			descentTime += lap.getDescentTime();		//12335: 2017-07-15
			distance += lap.getDistance();
		}
		pausedTime = elapsedTime - movingTime;			//12335: 2018-05-07
		...
		//12335: 2017-07-15 (start)
		if ( (this.maximumPositiveVerticalSpeed == null || recalculation) && maximumPositiveVerticalSpeed >= 0 )
			this.maximumPositiveVerticalSpeed = maximumPositiveVerticalSpeed;
		
		if ( (this.maximumNegativeVerticalSpeed == null || recalculation) && maximumNegativeVerticalSpeed >= 0 )
			this.maximumNegativeVerticalSpeed = maximumPositiveVerticalSpeed;
		
		if ( (this.averagePositiveVerticalSpeed == null || recalculation) && ascentTime > 0)
			this.averagePositiveVerticalSpeed = (float) (totalAscent / ascentTime);
		
		if ( (this.averageNegativeVerticalSpeed == null || recalculation) && descentTime > 0 )
			this.averageNegativeVerticalSpeed = (float) (totalDescent / descentTime);
		
		if ( (this.getAscentTime() == null || recalculation) && ascentTime > 0 )
			this.ascentTime = ascentTime;
		
		if ( (this.getDescentTime() == null || recalculation) && descentTime > 0 )
			this.descentTime = descentTime;
		//12335: 2017-07-15 (end)
	...
	//12335: 2017-03-26
	public Session clone( Activity parent) {
		Session session = new Session( parent);
		session.setStartTime( this.getStartTime());
		session.setEndTime( this.getEndTime());

		// other data that is not recalculated
		session.setTrigger( this.getTrigger());
		session.setEvent( this.getEvent());
		session.setEventType( this.getEventType());
		session.setEventGroup( this.getEventGroup());
		
		session.setCycles( this.getCycles());
		session.setStrides( this.getStrides());
		session.setCalories( this.getCalories());
		session.setFatCalories( this.getFatCalories());

		session.setAverageRunningCadence( this.getAverageRunningCadence());
		session.setMaximumRunningCadence( this.getMaximumRunningCadence());
		session.setAveragePower( this.getAveragePower());
		session.setMaximumPower( this.getMaximumPower());
