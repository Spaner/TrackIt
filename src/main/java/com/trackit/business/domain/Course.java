...
import com.trackit.business.common.ColorSchemeV2Container;
...
import com.trackit.business.operation.PauseDetectionOperation;
import com.trackit.business.utilities.PictureComparator;
import com.trackit.business.utilities.decoratedlist.EventList;
...
public class Course extends TrackItBaseType implements DocumentItem,
	...
	private Double ascentTime;						//12335: 2017-07-14
	private Double descentTime;						//12335: 2017-07-14
	...
	private DifficultyLevelType difficulty;			//12335: 2017-03-14
	private TrackConditionType trackCondition;		//12335: 2017-03-14
	private CircuitType circuitType;				//12335: 2017-03-15
...
//		unsavedChanges    = false;								// 58406
		...
		colorScheme       = TrackIt.getDefaultColorScheme();	// 58406
		noSpeedInFile     = true;//58406
		difficulty        = DifficultyLevelType.UNDEFINED;		// 12335: 2017-03-14
		trackCondition    = TrackConditionType.UNDEFINED;		// 12335: 2017-03-14
		circuitType       = CircuitType.OPEN;   				// 12335: 2017-03-15
	...
	// 12335: 2017-03-17 - Support for DB only data changed
	public boolean hasChangedDBOnlyData() {
		if ( sportOrSubSportChanged() || difficultyChanged() || trackConditionChanged()  ||
			 circuitTypeChanged() )
			return true;
		return false;
	...
	//12335: 2017-03-23 - handle null or empty fullFilepath
	...
	public List<Trackpoint> getTrackpoints(Date startTime, Date endTime) {
		...
		for (int i = 0; i < trackpoints.size(); i++) {
			...
			if (trackpoint.getTimestamp().equals(endTime)) {
				...
				System.out.println("Lap end time does not match trackpoint timestamp.");
				break;
			}
		}
...
//		setUnsavedTrue();// 58406	// 12335 : 2016-10-03 : using TrackStatus instead
		trackStatus.setTrackAsChanged();
		setCircuitType();			// 12335 : 2017-03-18
...
//		setUnsavedTrue();// 58406	// 12335 : 2016-10-03 : using TrackStatus instead
	...
	private void calculateSummaryData(ConsolidationLevel level) {
		...
		int totalAscent = 0;
		int totalDescent = 0;
		double ascentTime = 0;						//12335: 2017-07-14
		double descentTime = 0;						//12335: 2017-07-14
		double maximumPositiveVerticalSpeed = -1;	//12335: 2017-07-14
		double maximumNegativeVerticalSpeed = -1;	//12335: 2017-07-14
		...
		elapsedTime = (this.endTime.getTime() - this.startTime.getTime()) * .001;	//2018-05-06: 12335 (added)
		
		new PauseDetectionOperation().process( this);
		for(Pause pause : pauses){
			pausedTime += pause.getDuration();
		}
		for (Lap lap : laps) {
//			elapsedTime += lap.getElapsedTime();									//2018-05-06: 12335 (commented)
		...
		for (Lap lap : laps) {
			...
			//12335: 2017-07-14 (start)
			if ( lap.getMaximumPositiveVerticalSpeed() != null ) {
				maximumPositiveVerticalSpeed = Math.max( maximumPositiveVerticalSpeed, 
													     lap.getMaximumPositiveVerticalSpeed());
			}
			
			if ( lap.getMaximumNegativeVerticalSpeed() != null ) 
				maximumNegativeVerticalSpeed = Math.max( maximumNegativeVerticalSpeed, 
														 lap.getMaximumNegativeVerticalSpeed());
			
			if ( lap.getAscentTime() != null )
				ascentTime += lap.getAscentTime();
			
			if ( lap.getDescentTime() != null )
				descentTime += lap.getDescentTime();
			//12335: 2017-07-14 (end)
...
//			this.movingTime = this.timerTime - pausedTime;			// 2018-05-06: 12335 (replaced)
			this.movingTime = this.elapsedTime - pausedTime;
		}
		
		if ( this.ascentTime == null || recalculate )
			this.ascentTime = ascentTime;
		
		if ( this.descentTime == null || recalculate )
			this.descentTime = descentTime;
		...
		//12335: 2017-07-14 (start)
		if ( (this.maximumPositiveVerticalSpeed == null || recalculate) 
				&& maximumPositiveVerticalSpeed >= 0 )
			this.maximumPositiveVerticalSpeed = (float) maximumPositiveVerticalSpeed;
		
		if ( (this.maximumNegativeVerticalSpeed == null || recalculate )
				&& maximumNegativeVerticalSpeed >= 0 )
			this.maximumNegativeVerticalSpeed = (float) maximumNegativeVerticalSpeed;
		
		if ( (this.averagePositiveVerticalSpeed == null || recalculate)
				&& ascentTime > 0 )
			this.averagePositiveVerticalSpeed = (float) (totalAscent / ascentTime);
		
		if ( (this.averageNegativeVerticalSpeed == null || recalculate) 
				&& descentTime > 0 )
			this.averageNegativeVerticalSpeed = (float) (totalDescent / descentTime);
		//12335: 2017-07-14 (end)
	...
	public FileType[] getSupportedFileTypes() {
		return new FileType[] { FileType.GPX, /*FileType.FIT, */FileType.TCX,
				FileType.KML, FileType.CSV, FileType.NMEA };
	}
	...
	public void publishSelectionEvent(EventPublisher publisher) {
		EventManager
				...
				.publish(
						publisher,
						com.trackit.presentation.event.Event.COURSE_SELECTED,
						this);
	}
	...
	public void publishUpdateEvent(EventPublisher publisher) {
		EventManager
				...
				.publish(
						publisher,
						com.trackit.presentation.event.Event.COURSE_UPDATED,
						this);
	}
...
//		setUnsavedTrue();					// 12335 : 2016-10-03
	...
	//12335; 2017-03-23
	public void setDifficulty( List<Course> courses) {
		DifficultyLevelType difficulty = DifficultyLevelType.UNDEFINED;
		for( Course course: courses)
			if ( course.getDifficulty().getValue() > difficulty.getValue() )
				difficulty = course.getDifficulty();
		setDifficulty( difficulty);
	}
	
	//12335: 2017-03-14
	public void updateDifficulty( DifficultyLevelType difficulty) {
		setDifficulty( difficulty, true);
	}
	
	//12335: 2017-03-14
	private void setDifficulty( DifficultyLevelType difficulty, boolean isChange) {
		if ( this.difficulty != difficulty ) {
			this.difficulty = difficulty;
			for( Lap lap: laps)
				lap.setDifficulty( difficulty);
			if( isChange )
				this.publishUpdateEvent( null);
		}
	}
	...
	public boolean difficultyChanged() {
		Database database = Database.getInstance();
		return database.getDifficultyLevel( this) != difficulty.getValue();
	}
	...
	public void setTrackCondition( TrackConditionType trackCondition) {
		setTrackCondition( trackCondition, false);
	}
	
	//12335: 2017-03-23
	public void setTrackCondition( List<Course> courses) {
		TrackConditionType condition = TrackConditionType.UNDEFINED;
		TrackConditionType tmp;
		for( Course course: courses) {
			tmp = course.getTrackCondition();
			if ( tmp != TrackConditionType.UNDEFINED ) {
				if ( condition != TrackConditionType.UNDEFINED ) {
					if ( tmp.getValue() < condition.getValue() )
						condition = tmp;
				}
				else
					condition = tmp;
	...
	//12335: 2017-03-14
	public void updateTrackCondition( TrackConditionType trackCondition) {
		setTrackCondition( trackCondition, true);
	}

	//12335: 2017-03-14
	private void setTrackCondition( TrackConditionType trackCondition, boolean isChange) {
		if ( this.trackCondition != trackCondition) {
			this.trackCondition = trackCondition;
			for( Lap lap: laps)
				lap.setTrackCondition( trackCondition);
			if ( isChange )
				this.publishUpdateEvent( null);
		}
	}
	...
	//12335: 2017-03-14
	public boolean trackConditionChanged() {
		Database database = Database.getInstance();
		return database.getTrackCondition( this) != trackCondition.getValue();
	}
	...
	public void setCircuitType() {
		...
		if ( trackpoints.size() > 1 ) {
			Database database = Database.getInstance();
			tolerance = database.getJoinMergeDistanceTolerance( sport, subSport, false) * 10;
			this.circuitType = CircuitType.getType( getTrackpoints(), tolerance);
		}
		for( Lap lap: laps) {
			lap.setCircuitType( tolerance);
		}
	}
	
	//12335: 2017-03-18
	public boolean circuitTypeChanged() {
//123325: 2017-08-09
//		return (Database.getInstance().getCircuitType( this) == ((circuitType.getValue()) ? 1 : 0) );
		return (Database.getInstance().getCircuitType( this) == circuitType.getValue() );
	}
	...
	//57421
		...
		public Course copy() {
			...
			course.oldName = this.oldName;
			// trackpoints and coursepoints
			for( Trackpoint point: this.getTrackpoints() )
				course.getTrackpoints().add( point.clone( course));
			for( CoursePoint point: this.getCoursePoints() )
				course.getCoursePoints().add( point.clone( course));
			// laps
			for( Lap lap: this.getLaps() )
				course.getLaps().add( ((CourseLap) lap).clone( course));
			// tracks
			for( Track track: this.getTracks() ) {
				course.getTracks().add( ((CourseTrack) track).clone( course));
			}
			//

			// other settings
			course.setSportAndSubSport( this.getSport(), this.getSubSport());
			course.setDifficulty( this.getDifficulty());
			course.setTrackCondition( this.getTrackCondition());
		...
		public Course clone( GPSDocument parent, String nameSuffix) {
			Course course = new Course( this.getName() + nameSuffix);
			course.oldName = this.oldName + nameSuffix;
			course.setParent( parent);
			course.setFilepath( parent.getFileName());		
			// trackpoints and coursepoints
			for( Trackpoint point: this.getTrackpoints() )
				course.getTrackpoints().add( point.clone( course));
			for( CoursePoint point: this.getCoursePoints() )
			...
			for( Pause pause: this.getPauses())
				course.getPauses().add( pause.clone());
...
////			course.setTracks(this.tracks);							//12335: 2017-03-24
//			
//			List<Trackpoint> newPoints = new ArrayList<Trackpoint>();
//			for(Trackpoint trackpoint : this.getTrackpoints()){
////				Trackpoint newTrackpoint = trackpoint.clone();			//12335: 2017-03-24
////				newTrackpoint.setId(trackpoint.getId());				//12335: 2017-03-24
//				Trackpoint newTrackpoint = trackpoint.clone( course);	//12335: 2017-03-24
//				newPoints.add(newTrackpoint);
//			}
//			course.setTrackpoints(newPoints);
//			
////			course.setCoursePoints(this.coursePoints);					//12335: 2017-03-24
//			List<CoursePoint> coursePoints = new ArrayList<CoursePoint>();
//			for ( CoursePoint point : this.getCoursePoints() )
//				coursePoints.add( point.clone( course));
//			course.setCoursePoints( coursePoints);
//			
//			course.setSegments(this.segments);
//			course.setEvents(this.events);
//			
//			for( Track track: this.tracks )							//12335: 2017-03-24
//				course.add( ((CourseTrack) track).clone( course));
//			
...
////			course.setPauses(this.pauses);								//12335: 2017-03-24
//			for( Pause pause: this.pauses)
//				course.pauses.add( pause.clone());
...
//			
//			course.setSportAndSubSport( this.sport, this.subSport);		//12335: 2017-03-24
//			course.setDifficulty( this.difficulty);						//12335: 2017-03-24
//			course.setTrackCondition( this.trackCondition);				//12335: 2017-03-24
//			course.setCircuitType();									//12335: 2017-03-20
//			
//			return course;
//		}
