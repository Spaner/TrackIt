...
public class CoursePoint extends TrackItBaseType implements FolderTreeItem {
	...
	//12335: 2017-03-24
	public CoursePoint clone() {
		return clone( this.parent);
	}
	
	//12335: 2017-03-24
	public CoursePoint clone( Course parent) {
		CoursePoint point = new CoursePoint( this.name, this.type, parent);
		point.setTime( this.getTime());
		point.setDistance( this.getDistance());
		point.setLatitude( this.getLatitude());
		point.setLongitude( this.getLongitude());
		point.setAltitude( this.getAltitude());
		point.setNotes( this.getNotes());
		return point;
	}
