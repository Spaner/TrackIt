...
public class Connection {
	...
	public BufferedImage getImageFromURL(String url) {
		try {
//2018-05-08: 12335 - answer from some servers, e.g. OpenStreetMap,
//                     fails if the intermediate InputStream is used
//			InputStream resource = getResource(url);
//			if (resource == null) {
////				logger.debug(getMessage("connection.errorReadingResource"));	//12335: 2017-08-07
//				logger.debug(getMessage("connection.errorReadingResource", url));
//				return null;
//			}
//			
...
//			resource.close();
	...
	public InputStream getResource(String url) {
		...
		try {
			...
			e.printStackTrace();
		} catch (UnknownHostException e) {		//12335: 2017-08-07
			logger.error( "\n" + Messages.getMessage( "connection.unreachableOrUnknownHost", e.getMessage()));
		} catch (IOException e) {
			e.printStackTrace();
		}
