...
class SmartPaceMaker implements PaceMaker {
	...
	SmartPaceMaker(Course course, Map<String, Object> options) {
		this.course = course;
		this.sport = (String) options.get(Constants.SetPaceOperation.SPORT);
	}
	...
	public void setPace() {
		...
		for (Trackpoint trackpoint : course.getTrackpoints()) {
			...
			trackpoint.setSpeed(speed);
				...
				pos++;
				currentTimeMS += (long) (trackpoint.getTimeFromPrevious() * 1000);
				trackpoint.setTimeFromPrevious(trackpoint.getTimeFromPrevious());
				trackpoint.setTimestamp(new Date(currentTimeMS));
			}
		}
	}
	...
	private double calculateSpeed(Trackpoint trackpoint) {
		...
		if (sport.equals(getMessage("dialog.setPace.smartPaceCycling"))) {
			return getCyclingSpeed(grade);
		} else {
			return getHikingSpeed(grade);
		}
	}

	private double getCyclingSpeed(long grade) {
		Double speed = gradeSpeedMap.get(grade);
		return (speed == null ? MINIMUM_SPEED : speed);
	}

	private double getHikingSpeed(long grade) {
		int limit = 30;
		long limitGrade = grade;
		if (grade > limit) {
			limitGrade = limit;
		} else if (grade < -limit) {
			limitGrade = -limit;
		...
		double radianGrade = Math.toRadians(limitGrade);
		double dhdx = Math.tan(radianGrade);
		double exponent = -3.5 * Math.abs(dhdx + 0.05) + 0.175;
		calculatedSpeed = baseSpeed * Math.exp(exponent);
