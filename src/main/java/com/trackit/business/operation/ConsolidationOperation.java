...
public class ConsolidationOperation extends OperationBase implements Operation {
	private enum MissingDataField {
		LONGITUDE("LONGITUDE"), LATITUDE("LATITUDE"), ALTITUDE("ALTITUDE"), DISTANCE("DISTANCE"), TIMESTAMP(
				"TIMESTAMP"), SPEED("SPEED"), HEART_RATE("HEART_RATE"), POWER("POWER"), CADENCE("CADENCE"), TEMPERATURE(
						"TEMPERATURE");
	...
	private void setUp() {
		level = (ConsolidationLevel) options.get(Constants.ConsolidationOperation.LEVEL);
	}
	...
	private void processActivities(List<Activity> activities) throws TrackItException {
	...
	public void process(Activity activity) throws TrackItException {
		...
		//logger.debug(String.format("Consolidating activity %s.", activity.getName()));
		...
		new PauseDetectionOperation().process( activity);
		...
		if (activity.getTotalTimerTime() == null || activity.getStartTime() == null || recalculation) {
			...
			//12335: 2017-08-12
			if ( activity.getEndTime() == null || recalculation ) {
				activity.setEndTime( activity.getLastSession().getEndTime());
			}
		}
	}
...
//			course.setUnsavedChanges(unsaved);			// 12335 : 2016-10-03
	...
	public void process(Course course) throws TrackItException {
		...
		if (!validLapData(course) || level == ConsolidationLevel.RECALCULATION) {
			...
			double duration = (lastTrackpoint.getTimestamp().getTime() - firstTrackpoint.getTimestamp().getTime())
					/ 1000.0;
	...
	private void processTrackpoints(DocumentItem item) throws TrackItException {
		...
		//2016-06-04 : 12335 : get missing altitudes in one go instead of one at a time
		ArrayList<Location> locations = new ArrayList<>();
		for( Trackpoint pt : trackpoints)
			if ( pt.getAltitude() == null )
				locations.add( new Location( pt.getLongitude(), pt.getLatitude()));
		if ( !locations.isEmpty() ) {
			Elevation.fetchElevations( locations);
			int idx = 0;
			for ( Trackpoint pt : trackpoints )
				if ( pt.getAltitude() == null ) {
					pt.setAltitude( locations.get(idx).getAltitude());
					idx++;
				}
		}
		//2016-06-04 end
		...
		boolean positioningDataPresent = (previousTrackpoint.getLongitude() != null
				&& previousTrackpoint.getLatitude() != null);
		...
		while (iter.hasNext()) {
			...
			double meterPerSecondModifier = 3.6;
			if (prevSpeed == 0) {
				// prevSpeed = Constants.DEFAULT_SPEED_MS;
				if (item.isActivity()) {
					Activity activity = ((Activity) item);
					if (activity.getSport() == null || activity.getSubSport() == null) {
						DocumentManager.getInstance().loadSportFromDB(activity, true);
					}
					prevSpeed = activity.getSubSport().getDefaultAverageSpeed() / meterPerSecondModifier;
				} else if (item.isCourse()) {
					Course course = ((Course) item);
					if (course.getSport() == null || course.getSubSport() == null) {
						DocumentManager.getInstance().loadSportFromDB(course, true);
					}
					prevSpeed = course.getSubSport().getDefaultAverageSpeed() / meterPerSecondModifier;
				}
			...
			if (currentTrackpoint.getLongitude() == null || currentTrackpoint.getLatitude() == null) {
				iter.remove();
			}
		...
		if (noSpeedData) {
			//double limit = TrackIt.getPreferences().getDoublePreference(Constants.PrefsCategories.PAUSE, null,
					//Constants.PausePreferences.SPEED_THRESHOLD, 1.5);
			double limit = 0.0;
			if (item instanceof Activity){
				Activity activity = ((Activity) item);
				limit = activity.getSubSport().getPauseThresholdSpeed();
			}
	...
	@SuppressWarnings("unused")
	public static void processTrackpointsData(List<Trackpoint> trackpoints) throws TrackItException {
		Map<String, Boolean> missingData = calculateMissingData(trackpoints);
		processTrackpointsMissingData(trackpoints, missingData);
	}

	private static Map<String, Boolean> calculateMissingData(List<Trackpoint> trackpoints) {
	...
	private static Map<String, Integer> initializeFrequencies() {
	...
	private static void calculateFrequencies(Map<String, Integer> frequencies, List<Trackpoint> trackpoints) {
		for (Trackpoint trackpoint : trackpoints) {
	...
	private static Map<String, Boolean> calculateMissingData(Map<String, Integer> frequencies,
		...
		for (String field : frequencies.keySet()) {
			double percentage = frequencies.get(field) / (numberOfTrackpoints * 1.0);
	...
	private static void processTrackpointsMissingData(List<Trackpoint> trackpoints, Map<String, Boolean> missingData) {
		...
		while (iterator.hasNext()) {
			...
			for (String field : missingData.keySet()) {
				if (missingData.get(field)) {
					...
					boolean success = interpolate(field, trackpoints, trackpoints.indexOf(trackpoint));
					if (!success) {
	...
	private static boolean interpolate(String field, List<Trackpoint> trackpoints, int currentPosition) {
	...
	private static Trackpoint getPrevious(String field, List<Trackpoint> trackpoints, int currentPosition) {
		ListIterator<Trackpoint> iterator = trackpoints.listIterator(currentPosition);
	...
	private static Trackpoint getNext(String field, List<Trackpoint> trackpoints, int currentPosition) {
		ListIterator<Trackpoint> iterator = trackpoints.listIterator(currentPosition);
		Trackpoint trackpoint = null;
	...
	private static double interpolateDoubleValues(String field, Trackpoint previous, Trackpoint next) {
	...
	@SuppressWarnings("unused")
	private long interpolateLongValues(String field, Trackpoint previous, Trackpoint next) {
	...
	private static int interpolateIntegerValues(String field, Trackpoint previous, Trackpoint next) {
	...
	private static short interpolateShortValues(String field, Trackpoint previous, Trackpoint next) {
	...
	private static byte interpolateByteValues(String field, Trackpoint previous, Trackpoint next) {
	...
	private Date getRandomTimestamp() {
		...
		calendar.set(Calendar.YEAR, Utilities.getRandomNumber(2000, Calendar.getInstance().get(Calendar.YEAR)));
		calendar.set(Calendar.MONTH, Utilities.getRandomNumber(0, 11));
		calendar.set(Calendar.DAY_OF_MONTH,
				Utilities.getRandomNumber(1, calendar.getActualMaximum(Calendar.DAY_OF_MONTH)));
	...
	private void process(Trackpoint lastTrackpoint, Trackpoint currentTrackpoint, Double prevSpeed) {
		if (currentTrackpoint.getDistance() == null || level == ConsolidationLevel.RECALCULATION) {
			updateDistance(lastTrackpoint, currentTrackpoint);
		}
		currentTrackpoint.setDistanceFromPrevious(currentTrackpoint.getDistance() - lastTrackpoint.getDistance());

		if (currentTrackpoint.getTimestamp() == null || level == ConsolidationLevel.RECALCULATION) {
			updateTimestamp(lastTrackpoint, currentTrackpoint, prevSpeed);
		}
		currentTrackpoint.setTimeFromPrevious(
				(currentTrackpoint.getTimestamp().getTime() - lastTrackpoint.getTimestamp().getTime()) / 1000.0);
		...
		//2016-06-04: 12335: no longer needed, missing altitudes are now obtained by a single request
	...
	private void updateAltitude(Trackpoint trackpoint) {
		if (trackpoint.getLongitude() == null || trackpoint.getAltitude() != null) {
			return;
		}
	...
	private void updateDistance(Trackpoint lastTrackpoint, Trackpoint currentTrackpoint) {
	...
	private void updateTimestamp(Trackpoint previousTrackpoint, Trackpoint trackpoint, Double prevSpeed) {
		boolean keepTimes = TrackIt.getPreferences().getBooleanPreference(Constants.PrefsCategories.EDITION, null,
		...
		if (!keepTimes || trackpoint.getTimestamp() == null) {
			 ...
			 * DEFAULT_SPEED_MS;
			 */
			double timeFromPrevious = trackpoint.getDistanceFromPrevious() / prevSpeed * 1000;
			 ...
			 * Math.round((timeFromPrevious % 1) / 1000.0));
			 */
	...
	private void updateSpeed(Trackpoint lastTrackpoint, Trackpoint currentTrackpoint) {
		double speed = 0.0;
		if (currentTrackpoint.getTimeFromPrevious() > 0) {
			speed = currentTrackpoint.getDistanceFromPrevious() / currentTrackpoint.getTimeFromPrevious();
		}
		currentTrackpoint.setSpeed(speed);
	}

	private void calculateTrackpointGrades2(List<Trackpoint> trackpoints) throws TrackItException {
	...
	private void calculateTrackpointGrades(List<Trackpoint> trackpoints) {
		...
		for (int i = 0; i < trackpoints.size(); i++) {
			...
			double grade = resampledGrades[index] + (resampledGrades[index + 1] - resampledGrades[index]) * factor;

			trackpoint.setGrade((float) grade);
		}
	}

	private double[] getResampledGrades(List<Trackpoint> trackpoints, int segmentSize) {
		...
		for (int i = 1; i < numberOfSegments; i++) {
			...
			double previousAltitude = getAltitude(trackpoints, currentDistance - segmentSize);
			double nextAltitude = getAltitude(trackpoints, currentDistance);
			double grade = (nextAltitude - previousAltitude) / segmentSize * 100.0;
	...
	private double getAltitude(List<Trackpoint> trackpoints, double distance) {
		...
		for (int i = 1; i < trackpoints.size(); i++) {
			...
			if (currentTrackpoint.getDistance() >= distance) {
				...
				altitude = getAltitude(previousTrackpoint, currentTrackpoint, distance);
				break;
			}
		}
	...
	private double getAltitude(Trackpoint trackpoint1, Trackpoint trackpoint2, double distance) {
		double segmentDistance = trackpoint2.getDistance() - trackpoint1.getDistance();
		double factor = (distance - trackpoint1.getDistance()) / segmentDistance;
		double altitudeDifference = trackpoint2.getAltitude() - trackpoint1.getAltitude();
		double altitude = trackpoint1.getAltitude() + (altitudeDifference * factor);
	...
	private double calculateAverageTrackpointDistance(List<Trackpoint> trackpoints) {
		Trackpoint lastTrackpoint = trackpoints.get(trackpoints.size() - 1);
	...
	private void process(Lap lap) {
		ColorScheme colorScheme = (ColorScheme) lap.getParent()
				.getAttribute(Constants.PAINTING_ATTRIBUTES.COLOR_SCHEME);
		lap.setAttribute(Constants.PAINTING_ATTRIBUTES.COLOR_SCHEME, colorScheme);
	...
	private Lap createLap(Activity activity) {
		...
		for (Trackpoint trackpoint : trackpoints) {
			...
			if (trackpoint.getTimeFromPrevious() > 0) {
				averageSpeed += (trackpoint.getDistanceFromPrevious() / trackpoint.getTimeFromPrevious())
						* (trackpoint.getTimeFromPrevious() / time);
			}
			maximumSpeed = (trackpoint.getSpeed() > maximumSpeed ? trackpoint.getSpeed() : maximumSpeed);

			if (trackpoint.getHeartRate() != null && trackpoint.getHeartRate() > 0) {
		...
		lap.setEndLatitude(trackpoints.get(trackpoints.size() - 1).getLatitude());
		lap.setEndLongitude(trackpoints.get(trackpoints.size() - 1).getLongitude());
		lap.setEndAltitude(trackpoints.get(trackpoints.size() - 1).getAltitude());
		...
		lap.setAverageHeartRate(averageHeartRate > 0 ? (short) Math.round(averageHeartRate) : null);
		lap.setMinimumHeartRate(minimumHeartRate != 300 ? minimumHeartRate : null);
		lap.setMaximumHeartRate(maximumHeartRate > 0 ? maximumHeartRate : null);
		lap.setAverageCadence(averageCadence > 0 ? (short) Math.round(averageCadence) : null);
	...
	private void setColorScheme(DocumentItem item) {
		...
		item.setAttribute(Constants.PAINTING_ATTRIBUTES.COLOR_SCHEME, colorScheme);
	}
	...
	private void updateSpeedfromList(List<Trackpoint> trackpoints, int index) {
		...
		if (index > 0 && index < trackpoints.size() - 2) {
			...
			speed = (nextPoint.getDistance() - lastPoint.getDistance())
					/ (nextPoint.getTimeFromPrevious() + currentPoint.getTimeFromPrevious());
		}
		if (index == trackpoints.size() - 1) {
			lastPoint = trackpoints.get(index - 1);
			currentPoint = trackpoints.get(index);
			speed = currentPoint.getDistanceFromPrevious() / currentPoint.getTimeFromPrevious();
		}
		currentPoint.setSpeed(speed);
	}
	
	private void updateSpeedWithPauseTime(List<Trackpoint> trackpoints, double pauseLimit) {
		double vm;
		...
		double time = trkp.getTimestamp().getTime();
		double distancePlusOne = trkpPlusOne.getDistance();
		double timePlusOne = trkpPlusOne.getTimestamp().getTime();
		double distanceMinusOne, timeMinusOne;
		
		vm = ((distancePlusOne - distance)*1000) / (timePlusOne - time);
		
		if (vm > pauseLimit) { //2016:06:03 - 12335 - changed < to >, would not recognize initial pause
			trkp.setSpeed(-1.);
		...
		for (int i = 2; i < n; i++) {
			...
			time = trkp.getTimestamp().getTime();
			distanceMinusOne = trkpMinusOne.getDistance();
			timeMinusOne = trkpMinusOne.getTimestamp().getTime();

			//if (time - timeMinusOne == 0) {
				//vm = 0;
			//} else {
				vm = ((distance - distanceMinusOne)*1000) / (time - timeMinusOne);
			//}
		...
		for (int i = 1; i < n - 1; i++) {
			...
			timePlusOne = trkpPlusOne.getTimestamp().getTime();
			distanceMinusOne = trkpMinusOne.getDistance();
			timeMinusOne = trkpMinusOne.getTimestamp().getTime();
			if (trkp.getSpeed() < 0) {
				if (time != timeMinusOne && time != timePlusOne) {
					speed = (((time - timeMinusOne) * ((distancePlusOne - distance)*1000)) / (timePlusOne - time)
							+ ((timePlusOne - time) * ((distance - distanceMinusOne)*1000)) / (time - timeMinusOne))
					...
					if (time == timeMinusOne && time == timePlusOne) {
						...
						speed = ((distancePlusOne - distanceMinusOne)*1000) / (timePlusOne - timeMinusOne);
						trkp.setSpeed(speed);
					}
				}
			}
		...
		if (trkp.getSpeed() < 0) {
			...
			distancePlusOne = trkpPlusOne.getDistance();
			timePlusOne = trkpPlusOne.getTimestamp().getTime();
			speed = 2 * ((distancePlusOne - distance)*1000) / (timePlusOne - time) - trkpPlusOne.getSpeed();
			trkp.setSpeed(speed);
		}
		...
		if (trkp.getSpeed() < 0) {
			...
			distanceMinusOne = trkpMinusOne.getDistance();
			timeMinusOne = trkpMinusOne.getTimestamp().getTime();
			speed = 2 * ((distance - distanceMinusOne)*1000) / (time - timeMinusOne) - trkpMinusOne.getSpeed();
			trkp.setSpeed(speed);
		}
	}
