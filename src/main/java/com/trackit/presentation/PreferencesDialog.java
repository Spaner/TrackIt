...
import com.trackit.business.utilities.ConnectionProperties;
...
import com.trackit.presentation.utilities.swing.TrackItTextField;
...
import com.trackit.presentation.view.map.provider.armysurvey.MilitaryMapResolution;
...
public class PreferencesDialog extends JDialog implements EventPublisher {
	...
	private short ascentDescentClass;				// 2017-06-08: 12335
	...
	TreeMap<String, Short> sportsLabelsAndIds;
	TreeMap<String, Short> subSportsLabelsAndIds;
...
//		final JComboBox<String> subSportsChooser = new JComboBox<String>(model);
		final SportSubSportComboBoxes comboBoxes = new SportSubSportComboBoxes( activeSport, activeSubSport);
		final JComboBox<String> sportsChooser    = comboBoxes.getSportsComboBox();
		final JComboBox<String> subSportsChooser = comboBoxes.getSubSportsComboBox();
...
//				});
		...
		sportsChooser.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				saveSubSportPreferences( true);							// 12335: 2016-07-17
				comboBoxes.setSelectedSport();							// 12335: 2017-04-21
				activeSport = comboBoxes.getSport();					// 12335: 2017-04-21
				...
				activeSubSport = comboBoxes.getSubSport();				// 12335: 2017-04-21
		...
		subSportsChooser.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				saveSubSportPreferences( true);					// 12335: 2016-07-17
				comboBoxes.setSelectedSubSport();				// 12335: 2017-04-21
				activeSubSport = comboBoxes.getSubSport();		// 12335: 2017-04-21
				setSportValuesToInititialValues( true);
				defaultAverageSpeedModel.setValue( tempDefaultAverageSpeed);
				maximumAllowedSpeedModel.setValue( tempMaximumAllowedSpeed);
	...
	private void setSportValuesToInititialValues( boolean loadFromDatabase) {
		if ( loadFromDatabase ) {
			...
			ascentDescentClass				  = database.getAscentDescentClass( activeSport, activeSubSport); //2017-06-08: 12335
			activeSubSport.setValues(
					...
					currentGradeLimit, ascentDescentClass);
		}
...
// 12335: 2017-04-21
//		sportsLabelsAndIds = SportType.toString( database.getSportsIds());
//		String [] sportsList = sportsLabelsAndIds.keySet().toArray( new String[0]);
//		final JComboBox<String> sportChooser = new JComboBox<>( sportsList);
//		sportChooser.setSelectedItem( activeSport.toString());
		final SportSubSportComboBoxes comboBoxes = new SportSubSportComboBoxes( activeSport, activeSubSport);
		final JComboBox<String> sportChooser = comboBoxes.getSportsComboBox();
		sportChooser.setPreferredSize( new Dimension( 175, 16));

// 12335: 2017-04-21
//		subSportsLabelsAndIds = SubSportType.toString( database.getSubSportsIds( activeSport));
//		String [] subSportsList = subSportsLabelsAndIds.keySet().toArray( new String [0]);
...
//		subSportChooser.setSelectedItem( activeSubSport.toString());
		final JComboBox<String> subSportChooser = comboBoxes.getSubSportsComboBox();
...
//				activeSport = SportType.lookup( sportsLabelsAndIds.get( selectedSport));
//				subSportsLabelsAndIds = SubSportType.toString( database.getSubSportsIds( activeSport));
//				String [] subSportsList = subSportsLabelsAndIds.keySet().toArray( new String[0]);
...
//				activeSubSport = SubSportType.lookup( subSportsLabelsAndIds.get(selectedSubSport));
				comboBoxes.setSelectedSport();
				activeSport = comboBoxes.getSport();
				activeSubSport = comboBoxes.getSubSport();
			}
		});
		...
		btnReset.addActionListener( new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if ( subSportOnly.isSelected() ) {
					selected = 0;
					message = getMessage( "preferencesDialog.sportReset.resetSubSportWarning.text", activeSubSport.toString());
				}
				else if ( sportOnly.isSelected() ) {
					selected = 1;
					message = getMessage( "preferencesDialog.sportReset.resetSportWarning.text", activeSport.toString());
				}
	...
	private void updateSports(){
		...
		for(GPSDocument document : documents){
			...
			for(Course course : courses){
				...
				ascentDescentClass = database.getAscentDescentClass( sport, subSport);	//2017-06-08: 12335
