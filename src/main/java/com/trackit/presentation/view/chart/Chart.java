...
import com.trackit.presentation.view.chart.ChartView.ChartMode;
import com.trackit.presentation.view.chart.ChartView.ChartType;
import com.trackit.presentation.view.map.painter.MapPainterStyle;
...
class Chart extends JComponent {
	...
	private Axis xAxis;
	private Axis[] yAxis;
	protected PointerInfoDisplay pointerInfoDisplay;
	...
	void setDataSeries(List<DataSeries> dataSeries) {
		...
		validate();// 58406
		repaint();
		pointerInfoDisplay = new PointerInfoDisplay( chartView);
	}
...
//		if (!isInfoToDisplay()) {		//12335: 2017-07-17
		if (!isInfoToDisplay() && chart.pointerInfoDisplay != null )
			return;
...
//		displayInfo(graphics, info);
		...
		//12335: 2017-07-17 (start)
		Trackpoint trackpoint = null;
		if ( isDistanceMode() )
			trackpoint = chart.getChartView().getTrackpointAtDistance( value);
		else
			trackpoint = chart.getChartView().getTrackpointAtDuration( value);
		if ( trackpoint != null )
			chart.pointerInfoDisplay.show(graphics, trackpoint, mousePosition, chart.getWidth());
		//12335: 2017-07-17 (end)
