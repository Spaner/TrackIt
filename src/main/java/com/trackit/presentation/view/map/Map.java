...
import com.trackit.presentation.view.map.layer.EventsLayer;
...
public class Map extends JPanel {
	...
	public static enum MapMode {SIMPLE, SELECTION, EDITION, ANIMATION, ZOOMING, AREA/*58406*/, MULTI/*58406*/, LEGEND/*57421*/};
	...
	public void setMode(MapMode mapMode) {
		...
		removeLayer(MapLayerType.LEGEND_LAYER);//57421
		...
		//57421
		case LEGEND:
			Course course = DocumentManager.getInstance().getSelectedCourse();
			if (course==null) {
				break;
			}
			setCursor(targetCursor);
			addLayer(MapLayerType.HIGHLIGHT_LAYER);
			addLayer(MapLayerType.LEGEND_LAYER);
			DocumentManager.getInstance().setMapMode(mapMode);
			break;
		default:
	...
	private MapInfo getMapInfo(int x, int y) {
		...
		int height = getLayers().get(0).getHeight();		//12335: 2017-05-11
		
		x = Math.max(Math.min(x, getWidth()), 0);
//		y = Math.max(Math.min(y, getHeight()), 0);			//12335: 2017-05-11
		y = Math.max(Math.min(y, height), 0);
		
		mapInfo.setPointerX(x);
		mapInfo.setPointerY(y);
		
//		Location currentLocation = mapProvider.getLocation(x, y, getWidth(), getHeight());	//12335: 2017-05-11
		Location currentLocation = mapProvider.getLocation(x, y, getWidth(), height);
...
//					mapInfo.getCenterLongitude(), "Center")); // 2017-06-03: 12335
					mapInfo.getCenterLongitude(), Messages.getMessage( "mapView.center")));
		}
	}
